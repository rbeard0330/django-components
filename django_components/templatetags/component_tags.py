...
def do_component_block(parser, token):
    ...
    To pass many arguments without overly long lines:
        {% component_block "name" "positional_arg" keyword_arg="value" ... [only] %}
            {% component_args keyword_arg7="value"... %}
            ...
        {% endcomponent_block %}
    ...
    extra_arg_bits = []
    while tag_name == "component_args":
        extra_arg_bits += token.split_contents()[1:]
        tag_name, token = next_block_token(parser)

    component, context_args, context_kwargs = parse_component_args(parser, bits + extra_arg_bits, 'component_block')

    slots_filled = NodeList()
    while tag_name != "endcomponent_block":
        if tag_name == "slot":
            slots_filled += do_slot(parser, token, component=component)
        elif tag_name == "component_args":
            raise TemplateSyntaxError("component_args tags must be located immediately after the component_block tag")
        tag_name, token = next_block_token(parser)
